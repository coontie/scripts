...
foreach my $currentTeam (keys %hash) {
	my $graph = GraphViz->new(directed => 1, layout => 'dot', rankdir=>'LR', epsilon=>0, overlap => 'false', node => {shape => 'record'});
	#create a new graph output file
	open (OUTFILE, ">./$currentTeam.dot");
	...
	foreach my $currentSource (keys %{$hash{$currentTeam}}) {
		foreach my $currentDest (keys %{$hash{$currentTeam}{$currentSource}}) {
			foreach my $edgeLabel (keys %{$hash{$currentTeam}{$currentSource}{$currentDest}}) {
				my $sourceNode = $graph->add_node($currentSource,  height => $height, width => $width, color=>'blue');
				my $targetNode = $graph->add_node($currentDest, height => $height, width => $width, color=>'blue');
