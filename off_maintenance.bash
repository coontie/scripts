...
#echo "Setting the $PROD_ASG_NAME desired capacity to $DESIRED_CAPACITY"
#aws autoscaling set-desired-capacity --auto-scaling-group-name "$PROD_ASG_NAME" --desired-capacity $DESIRED_CAPACITY

#initialize to zero to force the while loop to execute
instances=`aws autoscaling describe-auto-scaling-groups --auto-scaling-group-name $PROD_ASG_NAME | grep InstanceId | awk '{print $2}' | sed s/\"//g | sed s/\,//g`
instanceCount=`echo $instances | wc -w`

echo "Found $instances in the $PROD_ASG_NAME"

#get the second field, remove trailing carriage return
echo "Discovering the ELBs attached to $MAINT_ASG_NAME"
elbList=$(aws autoscaling describe-auto-scaling-groups --auto-scaling-group-name $MAINT_ASG_NAME --output=text | grep LOADBALANCERNAMES | awk '{print $2}' | tr '\n' ' ')
echo "Discovered $elbList"

for elb in $elbList
do
        echo Attaching $elb to $PROD_ASG_NAME
        aws autoscaling attach-load-balancers --load-balancer-names "$elb" --auto-scaling-group-name "$PROD_ASG_NAME"
done
...
echo "Detaching $elbList from $MAINT_ASG_NAME"
for elb in $elbList
do
        echo detaching $elb
        aws autoscaling detach-load-balancers --load-balancer-names "$elb" --auto-scaling-group-name "$MAINT_ASG_NAME"
done
echo "Setting the $MAINT_ASG_NAME desired capacity to 0"
aws autoscaling set-desired-capacity --auto-scaling-group-name "$MAINT_ASG_NAME" --desired-capacity 0
